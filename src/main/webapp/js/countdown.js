...
var MARGIN_TOP = 10;
var MARGIN_LEFT = 5;
var endTime = new Date(2016, 2, 27, 18, 47, 52);
var curShowTimeSeconds = 0;
var balls = [];
var colors = ["#33B5E5", "#0099CC", "#AA66CC", "#9933CC", "#99CC00", "#669900", "#FFBB33", "#FF8800", "#FF4444", "#CC0000"];
window.onload = function() { WINDOW_WIDTH = document.getElementById("secondary").width;
    WINDOW_HEIGHT = document.getElementById("secondary").height;
...
function updateBalls() {
    for (var i = 0; i < balls.length; i++) { balls[i].x += balls[i].vx;
        ...
        if (balls[i].y >= WINDOW_HEIGHT - RADIUS) { balls[i].y = WINDOW_HEIGHT - RADIUS;
...
function addBalls(x, y, num) {
    for (var i = 0; i < digit[num].length; i++)
        for (var j = 0; j < digit[num][i].length; j++)
            if (digit[num][i][j] == 1) {
                var aBall = { x: x + j * 2 * (RADIUS + 1) + (RADIUS + 1), y: y + i * 2 * (RADIUS + 1) + (RADIUS + 1), g: 1.5 + Math.random(), vx: Math.pow(-1, Math.ceil(Math.random() * 1E3)) * 4, vy: -5, color: colors[Math.floor(Math.random() * colors.length)] };
                balls.push(aBall) } }
...
function render(cxt) {
    ...
    renderDigit(MARGIN_LEFT + 54 * (RADIUS +
        1), MARGIN_TOP, parseInt(minutes % 10), cxt);
    renderDigit(MARGIN_LEFT + 69 * (RADIUS + 1), MARGIN_TOP, 10, cxt);
    renderDigit(MARGIN_LEFT + 78 * (RADIUS + 1), MARGIN_TOP, parseInt(seconds / 10), cxt);
    renderDigit(MARGIN_LEFT + 93 * (RADIUS + 1), MARGIN_TOP, parseInt(seconds % 10), cxt);
    for (var i = 0; i < balls.length; i++) { cxt.fillStyle = balls[i].color;
        cxt.beginPath();
